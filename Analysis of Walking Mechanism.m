...
clear;
close;
%% Initialization of synthesized lengths, and arrays for angles
...
theta5 = 0*theta2;
theta6 = 0*theta2;
%% Solving for unknowns using fsolve
for i=1:N
...
for i=N:-1:1
    ...
    mod(i+floor(N/4),N+1)
    figure(1)
    plot(leg1(1,:),leg1(2,:), 'k-',LineWidth=2)
    hold on;
    plot(leg1(3,:),leg1(4,:), 'k-',LineWidth=2)
    plot(leg1(5,1:2),leg1(6,1:2),'k-',LineWidth=2)
    %Leg2
    plot(leg2(1,:)+15,leg2(2,:), 'k-',LineWidth=2)
    plot(leg2(3,:)+15,leg2(4,:), 'k-',LineWidth=2)
    plot(leg2(5,1:2)+15,leg2(6,1:2),'k-',LineWidth=2)
    %Leg3
    plot(leg3(1,:),leg3(2,:), 'k:',LineWidth=1.5)
    plot(leg3(3,:),leg3(4,:), 'k:',LineWidth=1.5)
    plot(leg3(5,1:2),leg3(6,1:2),'k:',LineWidth=1.5)
    %Leg4
    plot(leg4(1,:)+15,leg4(2,:), 'k:',LineWidth=1.5)
    plot(leg4(3,:)+15,leg4(4,:), 'k:',LineWidth=1.5)
    plot(leg4(5,1:2)+15,leg4(6,1:2),'k:',LineWidth=1.5)
    %Path traced
    ...
    axis("equal")
    grid on
    title("Pegasus Gait Mechanism (Trot) - Group 3",'FontSize',15,'FontName','Palatino Linotype')
    pause(0.0025)
end
